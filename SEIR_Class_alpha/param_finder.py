...
def met_hast(model,Ir,tr,beta_i,sigma_i,gamma_i,mu_i,r0,steps,err):
    ...
    for i in range(steps):
        print(i)
        [b_p,s_p,g_p,m_p]=transition_model(x["beta"],x["sigma"],x["gamma"],x["mu"],g*r0,model)
        x_new=model.integr_RK4(model.t0,model.T,model.h,b_p,s_p,g_p,m_p,False,True)
        e_n=objective_funct(Ir,tr,x_new["I"],x_new["t"],2)
        # Acceptance
        if(e_n/e_o<1):
           # p_v=np.array([b_p,s_p,g_p,m_p])-np.array([x['beta'],x['sigma'],x['gamma'],x['mu']])
            #v_n=LA.norm(p_v)
            #g=LA.norm(np.reciprocal(p_v))
            #d=e_o-e_n
            x=x_new
            #print(v_n)
            #print(d)
            ...
            g=e_0/e_n
            e_o = e_n
            #print([b_p,s_p,g_p,m_p,e_n])
            
            
        if(e_n<err):
            break
    return([x["beta"],x["sigma"],x["gamma"],x["mu"],e_n])
