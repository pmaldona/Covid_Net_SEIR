...
class SEIR :
        ...
        def __init__(self,P,eta,alpha,S0,E0,I0,R0,t0,T,h,beta_r,gamma_r,sigma_r,mu_r):
            ...
            self.beta=np.mean(beta_r)
            self.gamma=np.mean(gamma_r)
            self.sigma=np.mean(sigma_r)
        ...
        def integr_RK4(self,t0,T,h,beta,sigma,gamma,mu,fix=False,E0init=False):
            ...
            if(fix==True):
                ...
                return({"S":S,"E":E,"I":I,"R":R,"t0":t[0],"T":t[-1],"h":h,
                        "t":t,"beta":beta,"sigma":sigma,"gamma":gamma,"mu":mu})
